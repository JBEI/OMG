...
def model_TS_data():
    ...
    # return model_TS
    model = cobra.io.load_json_model(os.path.join(cwd,'omg/integration_tests/data/iJO1366_MVA.json'))
    return model
...
def test_get_flux_time_series(model_TS_data, solution_TS_data, user_params_data):
    ...
    solution_TS, model_TS, cell, Emets, Erxn2Emet = get_flux_time_series(model_TS_data, ext_metabolites, grid, user_params)
...
def test_get_proteomics_transcriptomics_data(model_TS_data, solution_TS_data):
    ...
    proteomics, transcriptomics = get_proteomics_transcriptomics_data(model_TS_data, solution_TS_data)
...
def get_multiomics(model_TS_data, solution_TS_data, user_params_data):
    pass
    proteomics, transcriptomics, metabolomics = get_multiomics(model_TS_data, solution_TS_data, user_params_data['mapping_file'], old_ids=True)
