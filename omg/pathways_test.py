...
def test_model_has_IPP_pathway():
    fname = os.path.join(os.path.dirname(__file__), 'integration_tests/data/iJO1366_test_with_IPP.json')
    model = cobra.io.load_json_model(fname)
    assert model_has_IPP_pathway(model) == True

def test_add_isopentenol_pathway():
    fname = os.path.join(os.path.dirname(__file__), 'integration_tests/data/iJO1366_test_wo_IPP.json')
    model = cobra.io.load_json_model(fname)
    sce_fname = os.path.join(os.path.dirname(__file__), 'integration_tests/data/iMM904.json')
