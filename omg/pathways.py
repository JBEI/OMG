...
def add_isopentenol_pathway(model, sce, user_params, write_file=True):
    ...
    model.reactions.get_by_id("PMD").add_metabolites(
        {"5dpmev_c": 1.0, "5pmev_c": -1.0, "ipdp_c": -1.0, "ipmp_c": 1.0}
    )
    ...
    model.add_reactions([r])
    r.add_metabolites(
        {"etoh_p": 1.0, "etoh_c": -1.0, "isoprenol_p": -1.0, "isoprenol_c": 1.0}
    )
    ...
    model.add_reactions([r])
    r.add_metabolites(
        {"etoh_e": 1.0, "etoh_p": -1.0, "isoprenol_e": -1.0, "isoprenol_p": 1.0}
    )
