...
def write_experiment_description_file(output_file_path, line_name="WT", label=""):
    ...
    # create the filename
    experiment_description_file_name = (
        f"{output_file_path}/EDD_experiment_description_file{label}.csv"
    )
    ...
    try:
        with open(experiment_description_file_name, "w") as fh:
            ...
            fh.write(
                f"{line_name}, {line_descr}, {part_id}, M9, 1, 0.1, 50, 200, 30, 1\n"
            )
    except Exception as ex:
...
def write_in_al_format(time_series_omics_data, omics_type, user_params, label=""):
    ...
    try:
        ...
        for timepoint, omics_dict in time_series_omics_data.items():
            al_file_name = (
                f"{output_file_path}/AL_{omics_type}_{timepoint}_hrs{label}.csv"
            )
...
def write_in_edd_format(
    time_series_omics_data, omics_type, user_params, line_name, label=""
):
...
def write_omics_files(
    ...
    if not al_format:
        ...
        write_in_edd_format(
            time_series_omics_data, omics_type, user_params, line_name, label=label
        )
